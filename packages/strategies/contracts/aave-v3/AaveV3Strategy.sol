...
import './IAaveV3Token.sol';
import './IAaveV3RewardsController.sol';

/**
 * @title AaveV3Strategy
 * @dev This strategy invests tokens in AAVE V3 lending pools, obtaining aTokens in exchange to accrue value over time.
 */
contract AaveV3Strategy is IStrategy, BaseImplementation {
    ...
    IAaveV3Token public immutable aToken;
    ...
    /**
     * @dev Creates a new AAVE V3 strategy contract
     * @param _token Token to be used as the strategy entry point
     * @param _lendingPool AAVE V3 lending pool to be used
     * @param _rewardsController AAVE V3 rewards controller contract
     * @param _registry Address of the Mimic Registry to be referenced
     */
    constructor(
        address _token,
        IAaveV3Pool _lendingPool,
        IAaveV3RewardsController _rewardsController,
        address _registry
    ) BaseImplementation(_registry) {
        IAaveV3Pool.ReserveData memory reserveData = _lendingPool.getReserveData(_token);
        require(reserveData.aTokenAddress != address(0), 'AAVE_V3_MISSING_A_TOKEN');

        token = _token;
        lendingPool = _lendingPool;
        rewardsController = _rewardsController;
        aToken = IAaveV3Token(reserveData.aTokenAddress);
    }
    ...
    function claim(bytes memory) external override returns (address[] memory tokens, uint256[] memory amounts) {
        ...
        return rewardsController.claimAllRewardsToSelf(aTokens);
    }
    ...
    function join(uint256 amount, uint256, bytes memory) external override returns (uint256 value) {
        ...
        lendingPool.supply(address(token), amount, address(this), 0);
    ...
    function exit(uint256 ratio, uint256, bytes memory) external override returns (uint256 amount, uint256 value) {
        ...
        require(ratio <= FixedPoint.ONE, 'AAVE_V3_INVALID_RATIO');
