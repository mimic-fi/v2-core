...
export function itBehavesLikeBalancerV2Connector(
  ...
  const source = SOURCES.BALANCER_V2
  ...
  context('single swap', () => {
    const data = defaultAbiCoder.encode(['bytes32'], [WETH_USDC_POOL_ID])
  ...
  context('batch swap', () => {
    context('USDC-WBTC', () => {
      const amountIn = toUSDC(10e3)
      const data = defaultAbiCoder.encode(['address[]', 'bytes32[]'], [[WETH], [WETH_USDC_POOL_ID, WETH_WBTC_POOL_ID]])
    ...
    context('WBTC-USDC', () => {
      ...
      const data = defaultAbiCoder.encode(['address[]', 'bytes32[]'], [[WETH], [WETH_WBTC_POOL_ID, WETH_USDC_POOL_ID]])
